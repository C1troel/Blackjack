...
public class BettingPickerController : MonoBehaviour
{
    ...
    {
        while (TestPlayerSpawner.Instance == null)
            yield return null;

        var playerSuit = TestPlayerSpawner.Instance.CurrentPlayerInfo;

        for (int i = 1; i < highestCardValue; i++)
        {
            var betCard = Instantiate(bettingCardPref, bettingPickerContainer.transform);
            betCard.GetComponent<OptionCard>().SetUpCardForBets(i, playerSuit.cardSuit);
        }
    }
}
