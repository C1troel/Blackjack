...
namespace Multiplayer
{
    namespace Panel
    {
        public class PanelScript : NetworkBehaviour
        {
            public enum Pos
            {
                None = -1,
            ...
            [SerializeField] private EffectPanelInfoMultiplayer _effectPanelInfo;
            ...
            [ClientRpc]
            public void HighlightAsPathEnderClientRpc(ClientRpcParams clientRpcParams = default)
            {
                GetComponent<SpriteRenderer>().sprite = SpriteLoadManager.Instance.GetPathEnderSprite();
            }

            [ClientRpc]
            public void RemoveHighlightClientRpc()
            {
            ...
            public EffectPanelInfoMultiplayer GetEffectPanelInfo => _effectPanelInfo;
            public int GetNeighboursCount => _posOfPanels.Count(obj => obj != null);

            public List<PanelScript> GetAvailableNearPanelsOrNull(PlayerController player, List<PanelScript> exceptionsPanels = null, PanelScript exceptionPanel = null, bool isDirectionMatter = false)
            {
                ...
                for (int i = 0; i < _posOfPanels.Count; i++)
                {
                    ...
                    else if (isDirectionMatter && (Pos)i == GetOppositePosOrNone((Pos)player.GetPlayerDirection))
                    {
                        continue;
                    }
            ...
            public (PanelScript, PlayerController.Direction) GetNextPanelOrNull(PlayerController player)
            {
                int availablePanelsCount = _posOfPanels.Count(panel => panel != null);
                PlayerController.Direction playerDirection = player.GetPlayerDirection;
            ...
            {
                initiator.StartMove((PlayerController.Direction)pos, panel);
                DeleteAllArrows();
            }
            ...
            private void OnTriggerEnter2D(Collider2D collision)
            {
                ...
                Debug.Log("PanelOnTriggerEnter2D");
                if (!IsServer)
                    return;
                ...
                if (collision.gameObject.CompareTag("Player"))
                {
                    ...
                    player.moveEndEvent += OnPlayerStay;
                }
            }
            ...
            private void OnMouseDown()
            {
                if (isClickableForTeleportation)
                {
                    ...
                    RequestForTeleportationServerRpc();
                }
            ...
            private void RequestForTeleportationServerRpc(ServerRpcParams serverRpcParams = default)
            {
                // код для перевірки того чи гравець взагалі може телепортуватися
                MapManager.Instance.StopWaiting();

                var playerInitId = serverRpcParams.Receive.SenderClientId;

                StartCoroutine(TestPlayerSpawner.Instance.TeleportPlayer(gameObject.transform.position, playerInitId, this));
            }

            private void OnTriggerExit2D(Collider2D collision)
            {
                Debug.Log("PanelOnTriggerExit2D");
                if (!IsServer)
                    return;
            ...
            private void OnPlayerStay(ulong playerId)
            {
                ...
                StartCoroutine(PanelEffectsManager.Instance
                    .TriggerPanelEffect(_effectPanelInfo.effect, playerId));
            }
